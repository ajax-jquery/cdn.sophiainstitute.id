...
!function(e){"use strict";const t="https://api.github.com",i=[[/^(https?):\/\/gitlab\.com\/([^\/]+.*\/[^\/]+)\/(?:raw|blob)\/(.+?)(?:\?.*)?$/i,"$1://gl.githack.com/$2/raw/$3"],[/^(https?):\/\/bitbucket\.org\/([^\/]+\/[^\/]+)\/(?:raw|src)\/(.+?)(?:\?.*)?$/i,"$1://bb.githack.com/$2/raw/$3"],[/^(https?):\/\/bitbucket\.org\/snippets\/([^\/]+\/[^\/]+)\/revisions\/([^\/\#\?]+)(?:\?[^#]*)?(?:\#file-(.+?))$/i,"$1://bb.githack.com/!api/2.0/snippets/$2/$3/files/$4"],[/^(https?):\/\/bitbucket\.org\/snippets\/([^\/]+\/[^\/\#\?]+)(?:\?[^#]*)?(?:\#file-(.+?))$/i,"$1://bb.githack.com/!api/2.0/snippets/$2/HEAD/files/$3"],[/^(https?):\/\/bitbucket\.org\/\!api\/2.0\/snippets\/([^\/]+\/[^\/]+\/[^\/]+)\/files\/(.+?)(?:\?.*)?$/i,"$1://bb.githack.com/!api/2.0/snippets/$2/files/$3"],[/^(https?):\/\/api\.bitbucket\.org\/2.0\/snippets\/([^\/]+\/[^\/]+\/[^\/]+)\/files\/(.+?)(?:\?.*)?$/i,"$1://bb.githack.com/!api/2.0/snippets/$2/files/$3"],[/^(https?):\/\/(?:cdn\.)?rawgit\.com\/(.+?\/[0-9a-f]+\/raw\/(?:[0-9a-f]+\/)?.+)$/i,"$1://gist.githack.com/$2"],[/^(https?):\/\/(?:cdn\.)?rawgit\.com\/([^\/]+\/[^\/]+\/[^\/]+|[0-9A-Za-z-]+\/[0-9a-f]+\/raw)\/(.+)/i,"$1://raw.githack.com/$2/$3"],[/^(https?):\/\/raw\.github(?:usercontent)?\.com\/([^\/]+\/[^\/]+\/[^\/]+|[0-9A-Za-z-]+\/[0-9a-f]+\/raw)\/(.+)/i,"$1://raw.githack.com/$2/$3"],[/^(https?):\/\/github\.com\/(.[^\/]+?)\/(.[^\/]+?)\/(?!releases\/)(?:(?:blob|raw)\/)?(.+?\/.+)/i,"$1://raw.githack.com/$2/$3/$4"],[/^(https?):\/\/gist\.github(?:usercontent)?\.com\/(.+?\/[0-9a-f]+\/raw\/(?:[0-9a-f]+\/)?.+)$/i,"$1://gist.githack.com/$2"],[/^(https?):\/\/git\.sr\.ht\/(~[^\/]+\/[^\/]+\/blob\/.+\/.+)/i,"$1://srht.githack.com/$2"],[/^(https?):\/\/hg\.sr\.ht\/(~[^\/]+\/[^\/]+\/raw\/.+)/i,"$1://srhgt.githack.com/$2"]];var a=e.getElementById("url-prod"),s=e.getElementById("url-dev"),r=e.getElementById("url");new Clipboard(".url-copy-button");var n=e.getElementById("url-dev-copy"),o=e.getElementById("url-prod-copy");function c(){var e=r.value=function(e){try{var e=new URL(e)}catch(t){return e}return e.pathname=e.pathname.replace(/\/\/+/gi,"/"),e.toString()}(decodeURIComponent(r.value.trim()));r.classList.remove("valid"),r.classList.toggle("invalid",e.length),s.value="",a.value="",s.classList.remove("valid"),a.classList.remove("valid"),n.disabled=!0,o.disabled=!0;var i=u(e);if(i){var c=i.match(/^(\w+:\/\/(raw).githack.com\/([^\/]+)\/([^\/]+))\/([^\/]+)\/(.*)/i);if(c){if("raw"===c[2]){s.value=i;let e=`${t}/repos/${c[3]}/${c[4]}/git/refs/heads/${c[5]}`;fetch(e).then(e=>{if(e.ok)return e.json()}).then(e=>{let t=e&&e.object&&e.object.sha?e.object.sha:c[5];document.getElementById("generateurl").value=l(`${c[1]}/${t}/${c[6]}`),a.value=l(`${c[1]}/${t}/${c[6]}`),d()})}}else s.value=i,document.getElementById("generateurl").value=l(i),a.value=l(i),d()}}function u(e){for(var t in i){var[a,s]=i[t];if(a.test(e))return e.replace(a,s)}}function l(e){return e.replace(/^(\w+):\/\/(\w+)/,"$1://$2cdn")}function d(){r.classList.remove("invalid"),r.classList.add("valid"),a.classList.add("valid"),s.classList.add("valid"),n.disabled=!1,o.disabled=!1}function p(e){setTimeout(function(){e.target.select()},1)}function h(e){e.classList.add("hidden")}function m(e){e.classList.remove("hidden")}e.queryCommandSupported&&e.queryCommandSupported("copy")&&(n.style.display="inline-block",o.style.display="inline-block"),r.addEventListener("input",c,!1),/(iPhone|iPad|iPod)/i.test(navigator.userAgent)&&(inputDev.removeAttribute("readonly"),inputProd.removeAttribute("readonly"),inputDev.addEventListener("keydown",function(e){e.preventDefault()}),inputProd.addEventListener("keydown",function(e){e.preventDefault()})),c(),a.addEventListener("focus",p),s.addEventListener("focus",p),r.addEventListener("focus",p);var v=e.querySelector(".purge textarea"),g=e.querySelector(".purge input[type=submit]"),$=e.querySelector(".purge .wait"),b=e.querySelector(".purge .success"),f=e.querySelector(".purge .error");autosize(v),v.oninput=function(){var e=[];for(var t of this.value.split("\n")){var i=u(t=decodeURIComponent(t.trim()));e.push(i?l(i):t)}return this.value=e.join("\n"),!1},document.getElementById("purge-form").onsubmit=function(){v.disabled=!0,g.disabled=!0,h(b),h(f),m($);var e="files="+encodeURIComponent(v.value);return fetch("/purge",{method:"POST",body:e}).then(e=>429==e.status?{success:!1,response:"too many requests"}:e.json()).then(e=>{h($),g.disabled=!1,v.disabled=!1;var t=e.success?b:f;t.textContent=e.response,m(t)}),!1}}(document);
